...
try:
    ...
    print(f"⚠️ Failed to load spaCy model: {e}. Keyword extraction disabled.")
    nlp = None
...
def extract_keywords(text: str) -> List[str]:
    """
    Extract keywords from text using spaCy or simple tokenization if spaCy is unavailable.
    """
    if nlp:
        doc = nlp(text.lower())
        ...
        common_stop_words = {"the", "a", "an", "and", "or", "in", "on", "at", "to"}
        return [word for word in words if word not in common_stop_words][:10]
...
Instructions:
1. Compare the task to the chunk summary, looking for specific keywords like 'meeting' for team tasks, 'report' for reporting tasks, or 'coding' for development tasks.
