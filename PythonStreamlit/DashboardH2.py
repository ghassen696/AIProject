...
def dashboard_page():
    ...
    daily_metrics = get_daily_metrics([employee_id], date_range[0], date_range[1])

    if not daily_metrics.empty and "summary" in daily_metrics.columns:
        st.subheader("📄 Chunk-level Summaries")
        for _, row in daily_metrics.sort_values(["date","chunk_index"]).iterrows():
            st.markdown(f"**{row['date']} – Chunk {row['chunk_index']}**: {row['summary']}")
    else:
        st.info("⚠ No summaries available for this employee in the selected range.")


    # --- Daily reports (one per day) ---
    st.subheader("📝 Daily Report")
    daily_reports = get_daily_reports(employee_id, date_range[0], date_range[1])
    if daily_reports:
        for date, summary in sorted(daily_reports.items()):
            st.markdown(f"**{date}**: {summary}")
    else:
        st.info("⚠ No daily reports available for this employee.")


    # --- Activity Breakdown (from employee_activity index) ---
    emp_data = df[df["employee_id"] == employee_id].copy()

    if not emp_data.empty:
        # Ensure timestamp is datetime
        emp_data["timestamp"] = pd.to_datetime(emp_data["timestamp"], errors="coerce")
        emp_data["date"] = emp_data["timestamp"].dt.date

        # Aggregate durations by event type per day
        if "duration_minutes" in emp_data.columns:
            daily_summary = emp_data.pivot_table(
                index="date",
                columns="event",
            ...
            value_cols = [c for c in ["active_minutes", "idle_minutes", "pause_minutes"] if c in daily_summary.columns]

            if value_cols:
                st.subheader("📊 Daily Activity Breakdown")
                st.plotly_chart(
                    px.bar(daily_summary, x="date", y=value_cols, barmode="group"),
                    use_container_width=True
                )

                st.subheader("⏱️ Active vs Idle vs Pause")
                st.plotly_chart(
                    px.bar(daily_summary, x="date", y=value_cols, barmode="stack"),
                    use_container_width=True
                )
            else:
                st.warning("⚠ No breakdown data available to plot.")
        else:
            st.warning("⚠ duration_minutes column not found in employee_activity index.")
    else:
        st.info("ℹ No employee activity data found for this range.")


    # --- Keystrokes & pastes trend ---
    if "num_keystrokes" in emp_data.columns and "num_pastes" in emp_data.columns:
        st.subheader("⌨️ Keystrokes & Clipboard Usage")
        st.plotly_chart(
            px.line(emp_data, x="date", y=["num_keystrokes", "num_pastes"], markers=True),
            use_container_width=True
        )

    # --- Window switching frequency ---
    if "num_window_switches" in emp_data.columns:
        st.subheader("🔄 Context Switching")
        st.plotly_chart(
            px.bar(emp_data, x="date", y="num_window_switches"),
            use_container_width=True
        )
