...
def get_active_window_title(): #check
        hwnd = win32gui.GetForegroundWindow()
        return win32gui.GetWindowText(hwnd) if hwnd else None

def is_sensitive_input(): #check
    try:
...
def flush_buffer(employee_id, window_name):#check
    global text_buffer
    with buffer_lock:
        if text_buffer:
...
def show_pause_dialog(icon):#check
    def on_submit():
...
def start_pause(reason, duration, icon):#check
    if paused.is_set():
        return
    paused.set()
...
def on_press(key, employee_id):#check
...
def on_release(key, employee_id):#check
...
def flush_if_inactive_loop(employee_id, window_name_getter, timeout=5):#check
